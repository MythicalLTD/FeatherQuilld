...
public sealed class WebSpaceUserAccessService
{
    ...
    public WebSpaceUserAccessService(IEventBus? events = null)
    {
    ...
    {
        var list = webspaceUuids as IReadOnlyList<Guid> ?? webspaceUuids.ToList();
        _events.WithHooks(
            new AccessDeauthorizeBeforeEvent { UserUuid = userUuid, WebSpaceUuids = list },
            err => new AccessDeauthorizeAfterEvent
            {
                UserUuid = userUuid,
                WebSpaceUuids = list,
                Error = err,
    ...
    public void SetPermissions(Guid userUuid, Guid webspaceUuid, IReadOnlyList<string> permissions) =>
        _events.WithHooks(
            new AccessSetPermissionsBeforeEvent
            {
                UserUuid = userUuid,
                WebSpaceUuid = webspaceUuid,
                Permissions = permissions,
            },
            err => new AccessSetPermissionsAfterEvent
            {
                UserUuid = userUuid,
                WebSpaceUuid = webspaceUuid,
                Error = err,
