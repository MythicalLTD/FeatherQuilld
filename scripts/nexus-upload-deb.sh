...
# Prod package must not use git-describe dirty versions (e.g. 0.1.0-5-gabcdef).
# Dev channel (~dev+) is allowed for featherquilld-dev only.
if [[ "${filename}" == featherquilld_* && "${filename}" != featherquilld-dev_* ]]; then
  if [[ "${NEXUS_BLOCK_DEV_VERSIONS:-1}" == "1" && "${filename}" =~ featherquilld_.*-g[0-9a-f]+_ ]]; then
    echo "Refusing to upload dirty git-describe version for prod package: ${filename}" >&2
    echo "Use a clean release tag version instead (e.g. 0.1.0)." >&2
    exit 3
  fi
fi
